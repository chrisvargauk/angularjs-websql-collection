...
// ## Update Models in collection ##
// #################################
// You can access all your models in your collection through cUser.JSON. You can access the first Model in the collection
// like this, cUser.JSON[0]. Feel free to update the value of any property, then all you need to do is call cUser.check().
// This method check for differences between the JSON and WebSQL and updates the database.
cUser.JSON[0].age = 23;
cUser.check();
...
//    itemCollectionType.defaultModel = JSON.parse(itemCollectionType.defaultModel);
  ...
  function saveExistingCollections() {
    listCmdSaveCollection.done(function() {
      ...
      copyCrawler(description, descriptionCopy);

      description = escapeCollection(descriptionCopy);
  ...
  function copyCrawler(obj, objCopy) {
    Object.keys(obj).forEach(function(key) {
      var value = obj[key];
...
// Collection as reusable component
angular.module('WebSQLCollection', []).factory('Collection', function () {
  return Collection;
});
